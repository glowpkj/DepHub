...
function Feature:_apply(character)
    ...
    pcall(function()
        if character:GetAttribute("WaterWalking") ~= true then
            character:SetAttribute("WaterWalking", true)
        end
    end)
end
...
function Feature:_bind(character)
    ...
    self.Context.Connect(self.CharacterConnections, character:GetAttributeChangedSignal("WaterWalking"), function()
        self:_apply(character)
    end)
end
