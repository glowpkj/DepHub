...
function Factory.new(context)
    ...
    function self:_removeVisual(player)
        ...
        if visual.Highlight then
            pcall(visual.Highlight.Destroy,visual.Highlight)
        end

        if visual.Billboard then
            pcall(visual.Billboard.Destroy,visual.Billboard)
        end
    end

    function self:_applyColor(player)
        local role,color=self:_role(player)
        local visual=self.Visuals[player]

        if not role or not color then
            if visual then self:_removeVisual(player) end
            return false
        end
    ...
    function self:_attachCharacter(player,character)
        ...
        if head then
            ...
            billboard.Size=UDim2.fromOffset(190,28)
        ...
        self:_debug("ESP "..role..": "..player.Name)
    end

    function self:_refreshPlayer(player)
        if self.Destroyed or not self.Enabled or player==LocalPlayer then return end

        local role=self:_role(player)
        ...
        if not visual or visual.Character~=character then
    ...
    function self:_watchPlayer(player)
        ...
        list[#list+1]=player.CharacterRemoving:Connect(function()
            self:_removeVisual(player)
        end)
