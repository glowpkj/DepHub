...
local old=PlayerGui:FindFirstChild("dephubViolenceDistrict")
...
Root.Position=UDim2.new(0.5,-Width/2,0.25,0)
...
local function makeToggle(parent,text,default,callback,sub)
    ...
    local function render(instant)
        ...
        if instant then
            ...
            TweenService:Create(switch,TweenInfo.new(0.12,Enum.EasingStyle.Quart,Enum.EasingDirection.Out),{BackgroundColor3=switchColor}):Play()
            TweenService:Create(dot,TweenInfo.new(0.12,Enum.EasingStyle.Quart,Enum.EasingDirection.Out),{Position=position}):Play()
        end
    end
    ...
    function control:Set(value,fire)
        state=value==true
        render(false)
        if fire~=false and type(callback)=="function" then task.spawn(callback,state) end
    ...
    track(row.MouseEnter:Connect(function()
        TweenService:Create(row,TweenInfo.new(0.08),{BackgroundColor3=sub and Color3.fromRGB(27,31,33) or Colors.Hover}):Play()
    end))
    track(row.MouseLeave:Connect(function()
        TweenService:Create(row,TweenInfo.new(0.08),{BackgroundColor3=sub and Colors.Sub or Colors.Control}):Play()
    end))
...
local function addFeature(text,default,callback)
    ...
    local function animate(open,instant)
        if activeTween then pcall(activeTween.Cancel,activeTween) activeTween=nil end
        local target=open and targetHeight() or 0
        if instant then
            settings.Size=UDim2.new(1,0,0,target)
            return
        end
        activeTween=TweenService:Create(settings,TweenInfo.new(0.2,Enum.EasingStyle.Quart,open and Enum.EasingDirection.Out or Enum.EasingDirection.In),{Size=UDim2.new(1,0,0,target)})
        activeTween:Play()
    end
    ...
    track(innerLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        if control:Get() then settings.Size=UDim2.new(1,0,0,targetHeight()) end
    end))
    task.defer(function()
...
addSection("Players")
addFeature("Player ESP",Backend:GetToggle("PlayerESP"),function(value)
...
addFeature("Auto Skill Check",Backend:GetToggle("AutoSkillCheck"),function(value)
    Backend:SetAutoSkillCheck(value)
end)

addSection("Diagnostics")
local debug,debugSettings=addFeature("Debug",Backend:GetToggle("Debug"),function(value)
...
track(RunService.Heartbeat:Connect(function(dt)
    if UI.Destroyed or not debug:Get() then return end
    ...
    DebugText.Text=
        ...
        "\nrepair remote: "..tostring(info.RepairRemote).." | skill remote: "..tostring(info.SkillRemote)..
        "\ngenerators: "..tostring(info.Generators).." | nearest: "..tostring(info.NearestPoint)..
        "\ndistance: "..string.format("%.1f",tonumber(info.NearestDistance) or 0).." | range: "..tostring(info.RepairRange)..
        "\ncurrent: "..tostring(info.CurrentGenerator).." / "..tostring(info.CurrentPoint)..
        "\nrepair calls: "..tostring(info.RepairCalls).." | skillchecks: "..tostring(info.SkillChecks)..
        "\nlast index: "..tostring(info.LastSkillIndex)..
...
function UI:SetOpen(state)
    ...
    if state then
        ...
        currentTween=TweenService:Create(Body,TweenInfo.new(0.16,Enum.EasingStyle.Quart,Enum.EasingDirection.Out),{Size=UDim2.new(1,0,0,BodyHeight)})
        currentTween:Play()
    else
        OpenButton.Text="OPEN"
        currentTween=TweenService:Create(Body,TweenInfo.new(0.13,Enum.EasingStyle.Quart,Enum.EasingDirection.In),{Size=UDim2.new(1,0,0,0)})
        local tween=currentTween
        tween:Play()
        task.spawn(function()
            tween.Completed:Wait()
            if not UI.Destroyed and not UI.Open and currentTween==tween then Body.Visible=false end
        end)
    end
end
...
track(Header.InputBegan:Connect(function(input)
    ...
    if point.X>=bp.X and point.X<=bp.X+bs.X and point.Y>=bp.Y and point.Y<=bp.Y+bs.Y then return end
    dragging=true
...
function UI:Destroy()
    ...
    if env.__DEPHUB_VD_FRONTEND==self then env.__DEPHUB_VD_FRONTEND=nil end
    if env.__DEPHUB and env.__DEPHUB.ViolenceDistrictUI==self then env.__DEPHUB.ViolenceDistrictUI=nil end
end
