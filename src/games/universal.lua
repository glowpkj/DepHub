...
function State:CreateUI()
    ...
    Movement:CreateSlider("Fly Speed", "Velocidade do voo.", 20, 250, 70, function(v) self.Values.FlySpeed = v end)
    Movement:CreateToggle("Fly", "WASD, Espaço e Ctrl.", false, function(v) self:SetFly(v) end)
    Movement:CreateToggle("No Clip", "Desativa a colisão do personagem.", false, function(v) self:SetNoclip(v) end)
    Movement:CreateButton("Adicionar Click Teleport", function() self:CreateTeleportTool() end)
    ...
    Visual:CreateToggle("Chams", "Destaque através de paredes.", true, function(v) self.Toggles.Chams = v end)
    Visual:CreateToggle("Cores dos times", "Usa a cor do time de cada jogador.", true, function(v) self.Toggles.TeamColors = v end)
    Visual:CreateColorPicker("Cor do ESP", "Cor usada quando Cores dos times está desligado.", self.Values.ESPColor, function(v) self.Values.ESPColor = v end)
    ...
    Visual:CreateSlider("Tamanho do texto", "Tamanho das informações.", 10, 30, 14, function(v) self.Values.ESPTextSize = v end)
    Visual:CreateToggle("Fullbright", "Clareia o mapa e remove neblina.", false, function(v) self:SetFullbright(v) end)
    Visual:CreateToggle("Infinite Zoom", "Amplia o limite da câmera.", false, function(v) self.Toggles.InfiniteZoom = v; LocalPlayer.CameraMaxZoomDistance = v and 100000 or self.OriginalZoom end)
