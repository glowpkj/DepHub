...
local function compile(source)
    ...
    if not ok or type(chunk)~="function" then
        return false,tostring(err or chunk)
    end
...
function State:HasExitLever()
    if self.Destroyed then return false end
    local feature=self.Features.Generator
    return feature and feature:HasExitLever() or false
end

function State:GetExitLeverInfo()
    if self.Destroyed then return {Available=false,Path="none"} end
    local feature=self.Features.Generator
    return feature and feature:GetExitLeverInfo() or {Available=false,Path="none"}
end

function State:TeleportToExitLever()
    if self.Destroyed then return false end
    local feature=self.Features.Generator
    return feature and feature:TeleportToExitLever() or false
end
